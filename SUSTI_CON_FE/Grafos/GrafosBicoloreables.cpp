...
void grafo::BFS(){
    ...
    while(!q.empty()){
        int k = q.front();
        q.pop();
        parista a = g[k]->adyacente;
        if(ord[k] == -1){
            ord[k] = cont++;
            while(a != NULL){
                if(ord[a->idxDestino] == -1) q.push(a->idxDestino);
                a = a->sgteArista;
            }
        }
    }
...
bool grafo::isBicolorable(int idx){
    queue<int> q;
    q.push(idx);
    color[g[idx]->idx] = 0;
    while(!q.empty()){
        ...
        while(a != NULL){
            ...
            else if(color[w] == color[k]) return false;
            a = a->sgteArista;
        }
    }
    return true;
...
int main(){
    ...
    g.insertarArista(2,0,6);

    g.imprimirGrafo();
    cout<<endl;
    
    cout<<"Es Bicolorable: "<<g.isBicolorable(0);
    cout<<endl;
